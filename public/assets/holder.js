...
var saveVcBtn = document.getElementById("saveVcBtn");
var clearAllBtn = document.getElementById("clearAllBtn");
var vcList = document.getElementById("vcList");
var signVpBtn = document.getElementById("signVpBtn");
var vpOutput = document.getElementById("vpOutput");
var audienceInput = document.getElementById("audience");
var nonceInput = document.getElementById("nonce");
var expiryInput = document.getElementById("expiry");
var copyVpBtn = document.getElementById("copyVpBtn");
var downloadVpBtn = document.getElementById("downloadVpBtn");
...
function renderList() {
  ...
  state.vcs.forEach((item) => {
    ...
    if (salts && typeof salts === "object") {
      ...
      note.textContent = "Legacy VC: kh\xF4ng c\xF3 merkleRoot/salts => kh\xF4ng th\u1EC3 ti\u1EBFt l\u1ED9 t\u1EEBng claim (h\xE3y t\u1EA1o l\u1EA1i VC Merkle).";
...
function normalizeStored(vc) {
  ...
  return { id: id2, raw: vc, issuer, subject, types, issuanceDate, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
}
...
function buildMerkleProofForVC(vc, claimKey) {
  ...
  const getValue3 = (k) => k === "id" ? subject?.id || vc?.holder || "" : subject?.[k];
  const keys = Object.keys(salts).sort();
  ...
  while (level.length > 1) {
    ...
    for (let i = 0; i < level.length; i += 2) {
      ...
      next.push(concatBytes3(L, R));
    }
    level = next;
    idx = Math.floor(idx / 2);
  }
  const root = level[0];
  return { root, proof, value: getValue3(claimKey), salt: salts[claimKey] };
}
